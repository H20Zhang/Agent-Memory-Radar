...
def native_record(
    identity: str = "native",
    radar_published_at: str = "2026-08-28T00:30:00Z",
    *,
...
def direction_line(
    ...
    *,
    ...
    synthesized: str = "2026-09-02T01:11:20Z",
    prior: str = "none",
...
def projection_readme(
    ...
    *,
    ...
    if language == "zh":
        seven_heading = "### 过去 7 天：2026-08-27—2026-09-02"
        thirty_heading = "### 过去 30 天：2026-08-04—2026-09-02"
    else:
        seven_heading = "### Last 7 days: 2026-08-27—2026-09-02"
        thirty_heading = "### Last 30 days: 2026-08-04—2026-09-02"
...
def prepend_timeline_entries(
    ...
    marker = '<a id="entry-2608-25329"></a>'
...
class CanonicalMemoryTimeContractTest(unittest.TestCase):
    ...
    def test_native_v2_requires_event_order(self):
        record = native_record()
        record["published_at"] = "2026-08-28T00:31:00Z"
        errors = validate_reading.validate_record_time_contract(record)
        self.assertTrue(
    ...
    def test_explicit_legacy_forbids_fabricated_discovery_or_radar_time(self):
        record = {
            ...
            "published": "2026-08",
            "published_at": "2026-08",
            "first_seen_at": "2026-09-02T01:11:20Z",
            "radar_published_at": None,
            "time_provenance": "legacy_unknown",
            "map_delta": "early_signal",
        }
    ...
    def test_schema_encodes_optional_native_direction_keys(self):
        ...
        native.update(
            ...
            first_seen_at="2026-09-02T01:11:20Z",
            radar_published_at="2026-09-02T01:11:20Z",
            time_provenance="native_v2",
            direction_keys=["memory-radar-acceptance-time"],
        )
...
class MemoryProjectionContractTest(unittest.TestCase):
    ...
    def test_native_timeline_rejects_acceptance_after_public_synthesis_cutoff(self):
        zh, en = valid_projection_pair()
        late = native_record("2608.90001", "2026-09-02T23:59:59Z")
    ...
    def test_periods_require_exact_windows_and_one_visible_range(self):
        ...
        records = repository_records()
        variants = {
            "wrong": zh.replace("2026-08-27—2026-09-02", "2026-08-13—2026-08-20", 1),
            "duplicate": zh.replace(
                direction_line("zh"),
                direction_line("zh") + " 2026-08-27—2026-09-02",
                1,
            ),
        }
        for name, mutated in variants.items():
    ...
    def test_period_metadata_and_visible_bindings_are_complete_and_bilingual(self):
        ...
        mutations = {
            ...
            .replace(
                "精确合成时间：`2026-09-02T01:11:20Z`",
                "精确合成时间：`2026-08-20`",
                1,
            )
            .replace(
                "仅原生接受时间可以支持窗口判断。",
                "仅原生接受时间可以支持窗口判断。旁注：`2026-09-02T01:11:20Z`。",
                1,
            ),
    ...
    def test_period_direction_blocks_require_exactly_one_of_each_visible_field(self):
        ...
        duplicates = {
            "zh": {
                ...
                "synthesis": "旁注：精确合成时间：`2026-09-02T01:11:20Z`。",
                "implication": (
            ...
            "en": {
                ...
                "synthesis": "Aside: Exact synthesis time: `2026-09-02T01:11:20Z`.",
                "implication": (
    ...
    def test_malformed_primary_fields_cannot_be_laundered_by_valid_labeled_asides(self):
        ...
        mutations = {
            ...
            "en": (
                ...
                (
                    "synthesis",
                    "Exact synthesis time: `2026-09-02T01:11:20Z`.",
        ...
        for state in ("splits", "retires"):
            ...
            cases = {
                ...
                "post-cutoff": (
                    ...
                    (identity,),
                    "field-map",
                    "accepted after direction synthesized=2026-09-02T01:11:20Z",
                ),
