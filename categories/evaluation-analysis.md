...
Memory evaluation is moving from **endpoint score → promotion evidence**. **D²ACCI** makes paired effects, protected slices, and stage-localizable traces part of feature promotion. **Competence, Not Accuracy** adds a precondition for reference-free skill gates: within-question discriminability on the actual optimization distribution, because marginal accuracy can be difficulty-confounded. **Explicit State Elicitation Is Not Enough** supplies the complementary negative result that interpretable state need not improve policy. **Demystifying Agent Skills** separates representation, retrieval, invocation, and execution. **Total Recall**, **AuthMem-Bench**, **SkillJack**, and **Practice Makes Unsafe** extend the vector to lifecycle cost, authority, provenance, and descendant-state harm.
...
**Biggest unresolved question:** what deployment-facing evidence vector can decide whether a memory mechanism or gate deserves promotion without collapsing causal stage, gate discriminability, user utility, lifecycle cost, protected-slice regressions, authorization, and descendant-state risk into one score?

**Next decisive evidence:** long-running tool-use deployments with paired interventions, within-item gate audits, stage traces, explicit protected slices, full offline+online cost, authority/revocation checks, and downstream action outcomes—plus intermediate states accepted only when they causally improve behavior.
