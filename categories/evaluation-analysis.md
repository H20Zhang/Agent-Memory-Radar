...
Memory evaluation is moving from **endpoint score → stage-localized promotion evidence**. **When Stale Constraints Go Unchecked** adds a particularly clean consumer-side test: historical provenance can be intact while the agent still fails because scarce verification is allocated away from the belief most costly to leave stale. At the same `k=2` budget, forcing the critical provenance path changes current-record-consistent decisions by 61.3–74.0pp across confirmatory/replicated settings while moving valid-world decisions by only 0–2pp. This isolates an allocation failure; the forced arm is an oracle instrument, not a deployable scheduler.

**D²ACCI** makes paired effects, protected slices, and stage-localizable traces part of feature promotion. **Competence, Not Accuracy** adds a precondition for reference-free skill gates: within-question discriminability on the actual optimization distribution, because marginal accuracy can be difficulty-confounded. **Remember, Verify, or Ask?** moves the authority check before writing, while **MemTrapBench** moves harm measurement after retrieval to the consumer's applicability decision. **Explicit State Elicitation Is Not Enough** supplies the complementary negative result that interpretable state need not improve policy. **Demystifying Agent Skills** separates representation, retrieval, invocation, and execution. **Total Recall**, **AuthMem-Bench**, **SkillJack**, and **Practice Makes Unsafe** extend the vector to lifecycle cost, authority, provenance, and descendant-state harm.

| Date | Paper | Tags | Importance | Research take |
|---|---|---|---:|---|
| 2026-08-26 | [When Stale Constraints Go Unchecked](../papers/2026/2608.25553.md) | `semantic` `structured` `general-agent` | 4/5 | Fixed-budget forced-critical verification identifies stale-memory harm as an allocation failure; the oracle critical path is not yet a deployable scheduler. |
...
| 2026-08-20 | [Remember, Verify, or Ask?](../papers/2026/2608.19564.md) | `semantic` `text` `personalization` `general-agent` | 3/5 | Commitment labels and structured tool choices diverge, but no write/check/question is executed and no later memory utility is measured. |
| 2026-08-19 | [Competence, Not Accuracy](../papers/2026/2608.18719.md) | `procedural` `text` `structured` `general-agent` | 4/5 | A judge gate must show within-question separation on genuine optimization traces; marginal AUC can overstate usable admission signal. |
...
**Biggest unresolved question:** what deployment-facing evidence vector can decide whether a memory mechanism or gate deserves promotion without collapsing causal stage, gate discriminability, commitment authority, freshness verification, useful-memory retention, trap rejection, lifecycle cost, protected-slice regressions, and descendant-state risk into one score?

**Next decisive evidence:** long-running tool-use deployments with paired interventions, executed commitment actions, mixed fresh/stale/memory-required/neutral/trap cases, learned verification allocation under a fixed budget, within-item gate audits, stage traces, explicit protected slices, full offline+online cost, authority/revocation checks, and downstream action outcomes.
