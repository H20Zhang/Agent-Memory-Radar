...
“Self-improving memory” now hides several different adaptive states. **SkillEvo** improves the feedback surface for iterative skill editing; **WER** trains the skill-writer policy itself from execution consequences; **TRUSS** uses controlled execution traces to certify and refine candidate procedural artifacts; **ERSkill** evolves the read policy/router; **HyperSkill** makes relational structure operational in retrieval and maintenance. The important question is no longer whether a system evolves, but **what state changes, from which evidence, and under what promotion/governance rule**.
...
**Biggest unresolved question:** which adaptive-state location—artifact, writer, read policy, structure, or governor—transfers across new consumers/domains strongly enough to justify rollout, verification, and maintenance cost?

**Next decisive evidence:** freeze source experiences, target tasks, and executor; independently vary feedback richness, writer learning, artifact refinement, read-policy evolution, certification, and governance under matched compute, then transfer the learned artifact/policy to a new domain without further tuning.
